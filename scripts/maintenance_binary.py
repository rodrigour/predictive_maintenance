...
def prep_train(m_df):
    ...
    binary = libraries.calculate_binary(rul)
...
test_prep_df, y_test = prepare_test(test_df, scale), libraries.calculate_binary(test_y)
...
for name, model in models:
    ...
    model.fit(train_prep_df, y_train['Binary'])
    train_pred = model.predict(train_prep_df)
    test_pred = model.predict(test_prep_df)
    train_acurracy = accuracy_score(y_train['Binary'], train_pred)
    test_acurracy = accuracy_score(y_test['Binary'], test_pred)
